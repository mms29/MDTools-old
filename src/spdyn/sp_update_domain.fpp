...
contains
  ...
  subroutine domain_interaction_update_md(istep, dynamics, ensemble, domain, &
    ...
    ! local variable
    integer                  :: ncell, nb, water_atom, i, ix
    real(dp)                 :: factor(3)
    ...
    if (mod(istep,dynamics%nbupdate_period) == 0) then
      ...
      if (dynamics%shrink_box) then
        if (dynamics%shrink_period > 0) then
          if (.not. ensemble%use_barostat .and. &
              mod(istep,dynamics%shrink_period) == 0) then
            ...
            factor(1) = boundary%box_size_x / boundary%box_size_x_ref
            factor(2) = boundary%box_size_y / boundary%box_size_y_ref
            factor(3) = boundary%box_size_z / boundary%box_size_z_ref
            ...
            call mpi_bcast(boundary%box_size_x, 1, mpi_real8, 0, &
                           mpi_comm_country, ierror)
            call mpi_bcast(boundary%box_size_y, 1, mpi_real8, 0, &
                           mpi_comm_country, ierror)
            call mpi_bcast(boundary%box_size_z, 1, mpi_real8, 0, &
                           mpi_comm_country, ierror)
            boundary%cell_size_x = boundary%box_size_x &
                                 / real(boundary%num_cells_x,dp)
            boundary%cell_size_y = boundary%box_size_y &
                                 / real(boundary%num_cells_y,dp)
            boundary%cell_size_z = boundary%box_size_z &
                                 / real(boundary%num_cells_z,dp)
            ...
            do i = 1, domain%num_cell_local+domain%num_cell_boundary
              do ix = 1, domain%num_atom(i)
                domain%coord(1:3,ix,i) = domain%coord(1:3,ix,i)*factor(1:3)
                domain%trans_vec(1:3,ix,i) = domain%trans_vec(1:3,ix,i)*factor(1:3)
              end do
            end do
         
            !if (main_rank) then
            !  write(MsgOut,'(a,3f15.8)') "check scalefactor", &
...
#ifdef DEBUG
      if (main_rank) then
        if (dynamics%shrink_box .and. .not. ensemble%use_barostat) then
          if ( mod(istep,dynamics%shrink_period) == 0) &
            write(MsgOut,'(a,3f15.8)') &
                 "check scalefactor", &
                 factor(1), factor(2), factor(3)
        endif
      ...
      if (boundary%type == BoundaryTypeNOBC) then
        ...
        call do_migration_cg(domain, enefunc, boundary, comm, pairlist)
  ...
  subroutine do_migration_cg(domain, enefunc, boundary, comm, pairlist)
  ...
  end subroutine do_migration_cg
