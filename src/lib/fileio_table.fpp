...
contains
  ...
  subroutine input_table(table_filename, table)
    ...
    ! read data
    call read_data_real      (f, 'cutoffdist',    table%cutoffdist)

    call read_data_real      (f, 'switchdist',    table%switchdist)

    call read_data_real      (f, 'dielec_const',  table%dielec_const)

    call read_data_real      (f, 'table_density', table%table_density)

    call read_data_integer   (f, 'table_order',   table%table_order)

    call read_data_real      (f, 'pme_alpha',     table%pme_alpha)

    call read_data_real_array(f, 'table_ene',  &
                               (/6*var_size/), table%table_ene)
    call read_data_real_array(f, 'table_grad', &
                               (/6*var_size/), table%table_grad)
    call read_data_real_array(f, 'table_ene_lj4',  &
                               (/2*var_size/), table%table_ene_lj4)
    call read_data_real_array(f, 'table_grad_lj4', &
                               (/2*var_size/), table%table_grad_lj4)
    call read_data_real_array(f, 'table_ecor',  &
                               (/  var_size/), table%table_ecor)
    call read_data_real_array(f, 'table_decor', &
  ...
  subroutine output_table(table_filename, use_ascii, table)
    ...
    ! write data
    call write_data_real      (f, 'cutoffdist',    table%cutoffdist)

    call write_data_real      (f, 'switchdist',    table%switchdist)

    call write_data_real      (f, 'dielec_const',  table%dielec_const)

    call write_data_real      (f, 'table_density', table%table_density)

    call write_data_integer   (f, 'table_order',   table%table_order)

    call write_data_real      (f, 'pme_alpha',     table%pme_alpha)

    call write_data_real_array(f, 'table_ene',  &
                               (/6*var_size/), table%table_ene)
    call write_data_real_array(f, 'table_grad', &
                               (/6*var_size/), table%table_grad)
    call write_data_real_array(f, 'table_ene_lj4',  &
                               (/2*var_size/), table%table_ene_lj4)
    call write_data_real_array(f, 'table_grad_lj4', &
                               (/2*var_size/), table%table_grad_lj4)
    call write_data_real_array(f, 'table_ecor',  &
                               (/  var_size/), table%table_ecor)
    call write_data_real_array(f, 'table_decor', &
                               (/  var_size/), table%table_decor)
