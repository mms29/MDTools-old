...
!-------------------------------------------------------------------------------
  ...
  function powersinh_double(x) result(y)
    ...
    a(2)  = a(1) / (4.0_dp*5.0_dp)
    a(3)  = a(2) / (6.0_dp*7.0_dp)
    a(4)  = a(3) / (8.0_dp*9.0_dp)
    a(5)  = a(4) / (10.0_dp*11.0_dp)
  ...
  function sum_gauss(n)
    ...
    ! formal arguments
    integer,    intent(in) :: n
    real(dp)               :: random_temp1, random_temp

    if( mod(n,2) .eq. 0 ) then
      sum_gauss = 2.0_dp*gamma_distr(n/2)
    else
      random_temp1 = 2.0_dp*gamma_distr((n-1)/2)
  ...
  function gamma_distr(n)
    ...
    do while(.true.)
      ...
      random1 = 2.0_dp*random_get() - 1.0_dp
      random2 = 2.0_dp*random_get() - 1.0_dp
      if (random1*random1 + random2*random2 .gt. 1.0_dp) cycle

      a = random2 / random1
      m = real(n-1,dp)
      b = sqrt(2.0_dp*m + 1.0_dp)
      c = b*a + m
    
      if ( c .le. 0.0_dp ) cycle

      d = (1.0_dp+a*a)*exp(m*log(c/m)-b*a)
