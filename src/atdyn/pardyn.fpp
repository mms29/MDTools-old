...
program pardyn
  ...
  character(MaxFilename)      :: log_filename
...
#ifndef MPI
  call error_msg("Must be run with MPI")
#endif
  ...
  if (main_rank) then
    ...
    write(MsgOut,'(A)')'****************************************************'
    write(MsgOut,'(A)')'*                                                  *'
    write(MsgOut,'(A)')'*                  GENESIS PARDYN                  *'
    write(MsgOut,'(A)')'*                                                  *'
    write(MsgOut,'(A)')'*        A Molecular Dynamics Simulator with       *'
    write(MsgOut,'(A)')'*              Parallel MD computation             *'
    write(MsgOut,'(A)')'*                                                  *'
    write(MsgOut,'(A)')'*                 Developed by RV                  *'
    write(MsgOut,'(A)')'*                                                  *'
    write(MsgOut,'(A)')'****************************************************'
    write(MsgOut,'(A)')' '
  endif
  ...
  call getarg(2,log_filename)
  call getarg(3,tmp_str)
  read(tmp_str, * ) pardyn_start
  
  ! CONTROL FILE
  tmp_int = index(ctrl_filename, "{}")
  if (tmp_int == 0) call error_msg("Use {} in INP file name")
  write(tmp_str,'(I0.5)') pardyn_start + my_world_rank
  ctrl_filename =  ctrl_filename(:tmp_int-1)//trim(tmp_str)//ctrl_filename(tmp_int+2:)

  ! LOG FILE
  tmp_int = index(log_filename, "{}")
  if (tmp_int == 0) call error_msg("Use {} in INP file name")
  write(tmp_str,'(I0.5)') pardyn_start + my_world_rank
  ...
  if (file_exists) then
      ...
      call open_file (unit_no=unit_no, filename=log_filename, in_out=IOFileOutputNew)
  endif
  DynvarsOut = unit_no
  DynvarsOut = unit_no
